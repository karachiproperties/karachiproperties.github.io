...
if(finderGo){
  finderGo.addEventListener('click',()=>{
    ...
    if(s)p.set('sector',s); if(z)p.set('size',z); if(st)p.set('status',st);
...
async function initRates(){
 ...
 const sectorValues=[...new Set(rows.map(r=>r.parent_sector))].sort((a,b)=>parseInt(a)-parseInt(b));
 ...
 function render(){
   let x=rows.filter(r=>(!sector.value||r.parent_sector===sector.value)&&(!size.value||String(r.plot_size)===size.value)&&(!status.value||r.status===status.value)&&(!q.value||r.area_code.toLowerCase().includes(q.value.toLowerCase())));
   tbody.innerHTML=x.map(r=>`<tr><td><a href="${esc(r.seo_url)}"><strong>${esc(r.area_code)}</strong></a></td><td>${r.plot_size} Sq Yds</td><td><span class="status ${r.status==='Possession'?'pos':'dev'}">${esc(r.status)}</span></td><td>${r.min_lac==null?'<span class="muted">Contact for verified rate</span>':`${r.min_lac}–${r.max_lac} Lac`}</td><td><a href="https://wa.me/923312608959?text=${encodeURIComponent('Assalam o Alaikum, '+r.area_code+' '+r.plot_size+' yards ka latest verified rate bata dein.')}" target="_blank" rel="noopener"><strong>Get rate →</strong></a></td></tr>`).join('') || '<tr><td colspan="5">No matching areas found.</td></tr>';
   document.querySelector('#result-count').textContent=x.length+' matching records';
   const p=new URLSearchParams();
   if(sector.value)p.set('sector',sector.value);if(size.value)p.set('size',size.value);if(status.value)p.set('status',status.value);if(q.value)p.set('q',q.value);
 ...
 document.querySelector('#clear-filters')?.addEventListener('click',()=>{q.value='';sector.value='';size.value='';status.value='';render();});
 render();
}
initRates();
